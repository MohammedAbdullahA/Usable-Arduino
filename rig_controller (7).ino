 ...
 *
 * Three preset profiles run on top of the free-control commands above:
 ...
 *   PROFILE_EC: fixed preset — Motor 1 forward 71000° at 600 RPM (see
 *     startMoveTo1/cmdProfileEC), meant to be run from the home position.
 *   All are abortable by STOP/ESTOP like any other motion. See cmdProfileWT,
 *   updateProfileWT, cmdProfileUT, and cmdProfileEC.
...
const float STEPS_PER_REV        = 1600.0f;  // Motor 1 (DM860H-class, back-and-forth axis)

// Motor 2 (TB6600-class generic driver, rotary axis) has its own DIP switch
// table and microstep setting — must be set independently of Motor 1.
// Check the table printed on that driver and set this to match exactly,
// otherwise WT/ROTARY/HOME degree calculations will be wrong.
// Default: 1600 (8x microstepping, SW1=OFF SW2=ON SW3=OFF per that driver's label).
const float STEPS_PER_REV_2      = 1600.0f;  // Motor 2 (generic driver, rotary axis)
...
const float MOTOR2_ACCEL_DEF     = 40000.0f;  // raised from 1500 (a leftover AccelStepper-era default) — Motor 2 now has real Timer2 hardware capability and needs a much faster ramp to actually reach speed within a short move (e.g. a 90° WT step) or to reach cruise quickly in PROFILE_UT; kept below Motor 1's 80000 since this is a smaller NEMA17
...
//
// CRITICAL: even at the largest prescaler (1024), the smallest frequency
// Timer2 can represent at all is F_CPU/(2*1024*256) ≈ 30.5 Hz — below
// that, computeTimer2Config returns false and timer2SetFrequency does
// nothing, leaving the hardware clock-select bits unset (Timer2 not
// running) while software position tracking (updateSpin2Position, which
// integrates spin2CurrentFreq × time) keeps advancing regardless. That
// mismatch — software believing steps happened that the hardware never
// actually generated — is exactly the kind of bug that makes a commanded
// move land far short of its target. TIMER2_MIN_FREQ below is the floor
// every Motor 2 ramp is clamped to, so this can never happen: the very
// first frequency ever asked of Timer2 is already representable.
const float TIMER2_MIN_FREQ = 40.0f;  // Hz — safe margin above the ~30.5Hz hard floor
...
void startMoveTo2(long steps, float cruiseFreq, int kind) {
  ...
  spin2CurrentFreq = TIMER2_MIN_FREQ;  // Timer2 can't represent anything below this — see the note above computeTimer2Config
...
void updateSpin2Ramp() {
  ...
  if (moveTo2Active) {
    ...
    if (effectiveTarget < TIMER2_MIN_FREQ) effectiveTarget = TIMER2_MIN_FREQ;  // never ask Timer2 for a frequency it can't represent
    ...
    if (freq < TIMER2_MIN_FREQ) freq = TIMER2_MIN_FREQ;
    spin2CurrentFreq = freq;
    timer2SetFrequency(freq);
    return;
  }
...
void processCommand(char* line) {
  ...
  if (strcmp(command, "STOP") == 0) {
    ...
    cmdProfileWT(argStr);
  } else if (strcmp(command, "PROFILE_EC") == 0) {
    cmdProfileEC();
...
// Launches a Timer1 position-controlled move on Motor 1: signed relative
// `steps`, target cruise speed `cruiseFreq`. Shared by MOVE_TO and
// PROFILE_EC — same mechanism, just a different way of arriving at the
// steps/speed to use.
void startMoveTo1(long steps, float cruiseFreq) {
  spinDirection = (steps > 0) ? 1 : -1;
  digitalWrite(MOTOR1_DIR_PIN, steps > 0 ? HIGH : LOW);

  spinCruiseFreq = cruiseFreq;
...
// branch; this function just validates input and hands off to startMoveTo1.
void cmdMoveTo(char* argStr) {
  ...
  startMoveTo1(steps, freq);

  Serial.print(F("OK "));
  Serial.println(lastCommandText);
}

// PROFILE_EC — fixed preset: moves Motor 1 (the linear/back-and-forth
// axis) forward 71000° at 600 RPM, meant to be run starting from the home
// position. Same Timer1 mechanism as MOVE_TO (startMoveTo1) — this is
// purely a one-click shortcut for a specific, frequently-used move, not a
// new motion capability.
const float PROFILE_EC_DEG = 72000.0f;
const float PROFILE_EC_RPM = 600.0f;

void cmdProfileEC() {
  if (faultActive) { Serial.println(F("ERROR FAULT_ACTIVE")); return; }
  if (busy)         { Serial.println(F("BUSY")); return; }

  float freq = PROFILE_EC_RPM * STEPS_PER_REV / 60.0f;
  long steps = lround((double)PROFILE_EC_DEG * STEPS_PER_REV / 360.0);
...
void cmdProfileUT(char* argStr) {
  ...
  float rotaryFreq = rotaryRpm * STEPS_PER_REV_2 / 60.0f;
  float linearFreq = linearRpm * STEPS_PER_REV / 60.0f;
  if (rotaryFreq > SPEED_MAX || linearFreq > SPEED_MAX) {
    Serial.println(F("ERROR OUT_OF_RANGE"));
    return;
  }
  if (rotaryFreq < TIMER2_MIN_FREQ) {
    // Motor 2's Timer2 hardware genuinely can't represent anything below
    // this (see the note above computeTimer2Config) — reject rather than
  ...
  spin2CurrentFreq = TIMER2_MIN_FREQ;  // Timer2 can't represent anything below this
  spin2StoppingSoft = false;
  moveTo2Active = false;
  motor2MoveKind = 0;
  spin2RampLastMillis = millis();
...
void cmdProfileWT(char* argStr) {
  ...
  float freq = rpm * STEPS_PER_REV_2 / 60.0f;
  if (freq > SPEED_MAX) {
    Serial.println(F("ERROR OUT_OF_RANGE"));
    return;
  }
  if (freq < TIMER2_MIN_FREQ) {
    Serial.println(F("ERROR OUT_OF_RANGE"));
    return;
  }
  ...
  profileWTWaiting = false;
  profileWTStep = 0;
  long steps = lround((double)PROFILE_WT_DEG * STEPS_PER_REV_2 / 360.0);
