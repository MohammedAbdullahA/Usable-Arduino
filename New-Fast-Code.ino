 ...
 *     Controlled via SPIN (continuous, by RPM), ROTATE (fixed angle at
 *     whatever speed is currently set), or MOVE_TO (fixed angle at a
 *     specified RPM in one command — the "run to the far end at speed X"
 *     command).
...
// as ROTATE: sign gives direction) at a specified cruise speed, then stops
// automatically on arrival. This is the one-shot "run the jig from one end
// to the other at a chosen speed" command — ROTATE alone doesn't take a
// speed, and SPIN alone has no target. rpm sets motor1's max speed (this
// persists afterward too, same as SET_SPEED_1 would) and must be positive;
// direction comes only from the sign of degrees.
void cmdMoveTo(char* argStr) {
  ...
  float speedStepsPerSec = rpm * STEPS_PER_REV / 60.0f;
  if (rpm <= 0 || speedStepsPerSec > SPEED_MAX) {
    Serial.println(F("ERROR OUT_OF_RANGE"));
    return;
  }

  motor1.setMaxSpeed(speedStepsPerSec);
  long steps2 = lround((double)degrees * STEPS_PER_REV / 360.0);
  motor1.move(steps2);
  busy = true;
  movingMotor1 = true;
  movingMotor2 = false;
