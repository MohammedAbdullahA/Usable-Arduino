 ...
 *   PROFILE_WT: Motor 2 does 4x (rotate 90°, wait 8s) = one full turn.
 *   PROFILE_UT: Motor 2 spins continuously at its configured max speed
 *     while Motor 1 drives toward D6/MIN at exactly 2x Motor 2's step
 *     rate — both motors run 1600 steps/rev, so a 2x step-rate ratio is
 *     exactly the industry-standard 2:1 rotation ratio (one wrap every
 *     5mm of linear travel). Both stop together the instant D6 triggers.
...
void cmdProfileUT() {
  ...
  // until D6/MIN triggers, at exactly 2x Motor 2's step rate.
  float linearSpeed = motor2Speed * 2.0f;
  if (linearSpeed > SPEED_MAX) linearSpeed = SPEED_MAX;  // clamp to the hardware ceiling if 2x would exceed it
  spinDirection = -1;
  digitalWrite(MOTOR1_DIR_PIN, LOW);
  spinTargetFreq = linearSpeed;
...
// PROFILE_WT — Motor 2 does PROFILE_WT_REPS x (rotate PROFILE_WT_DEG°, wait
// PROFILE_WT_WAIT_MS), completing one full revolution in quarter-turn
// steps, on the Timer2 hardware path (startMoveTo2, kind=WT_STEP). The
// timing between turns is tracked here in updateProfileWT(), called every
// loop() iteration so the rig stays responsive to STOP/ESTOP throughout.
void cmdProfileWT() {
  if (faultActive) { Serial.println(F("ERROR FAULT_ACTIVE")); return; }
  if (busy)         { Serial.println(F("BUSY")); return; }
  ...
  startMoveTo2(steps, motor2Speed, 3 /* WT_STEP */);
